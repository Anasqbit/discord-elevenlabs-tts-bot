...
# ===== الحدث المعدل - تم إزالة قيد 100 حرف =====
@bot.event
async def on_message(message: discord.Message):
    ...
    if message.content.startswith('-'):
        ...
        # تم إزالة التحقق من طول النص هنا
...
async def say(interaction: discord.Interaction, text: str):
    try:
        ...
        # تم إزالة التحقق من طول النص هنا أيضاً
        ...
        if audio_stream:
            ...
            if not interaction.guild.voice_client.is_playing():
                bot.play_next_audio(interaction.guild)
            await interaction.followup.send("تمت إضافة الرسالة إلى الطابور")
        else:
            await interaction.followup.send("فشل توليد الصوت.")
...
async def ensure_voice_connection(interaction: discord.Interaction):
    if interaction.user.voice:
        ...
        await interaction.followup.send('يجب أن تكون في قناة صوتية!')
